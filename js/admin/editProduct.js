...
async function editFormHandler(product) {
    if (product.id === "") {
        document.getElementById("alertEditForm").innerHTML =
    ...
    if (product.name === "") {
        document.getElementById("alertEditForm").innerHTML =
            "<div class='alert alert-danger'>Please enter new product's name</div>";
        return false;
    }

    if (product.price === "") {
        document.getElementById("alertEditForm").innerHTML =
            "<div class='alert alert-danger'>Please enter new product's price</div>";
        return false;
    }

    if (product.desc === "") {
        document.getElementById("alertEditForm").innerHTML =
            "<div class='alert alert-danger'>Please enter new product's description</div>";
        return false;
    }

    await axios
        .put("http://localhost:5500/api/products/update_product.php", product, {
            headers: {
